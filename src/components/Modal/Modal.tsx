...
const Modal = () => {
  ...
  return (
    <div
      className={
        showModal
          ? `modal top-0 left-0 w-full h-full bg-black/40 z-[1000] ${
              !loading &&
              modalContent.content != "write-review" &&
              "flex justify-end"
            } ${
              !loading &&
              modalContent.content == "write-review" &&
              "flex justify-center items-center"
            } transition-all duration-[1.5s] fixed`
      ...
      {modalContent.title && (
        <div
          className={`fixed mx-auto ${
            modalContent.content == "write-review" ? "h-fit" : "h-full"
          } overflow-hidden bg-white md:w-[500px] w-[90%]`}
        >
