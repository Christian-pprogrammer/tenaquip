...
import ShippingOption from "@/components/shipping-option/ShippingOption";
import { setShowModal } from "@/Store/slices/modal";
import { setLoading } from "@/Store/slices/loading";
import { Paypal } from "@/components/paypal/Paypal";

const stripePromise = loadStripe(
  "pk_test_51JFxudBy7J9W92jTmsSCvnTdcD9NNq72zIyx7OYEWiHEoRvqP5Dmn6v80ZAqIQomXf45bcT58f2h024uQuzDR4UO00QUKCbj2a"
);
...
const Review = () => {
  ...
  const paymentMethods: Array<{ title: string; images: Array<string> }> = [
    ...
    {
      title: "PayPal",
      images: ["https://www.tenaquip.com/images/icon/pp-logo-100px.png"],
    },
  ];
  ...
  useEffect(() => {
    ...
    async function createStripePaymentSession() {
      try {
        const payment_sessions = await axios.post(
          `${process.env.MEDUSA_BACKEND_API}/store/carts/${cart.id}/payment-sessions`,
          {}
        );
        ...
        const res = await axios.post(
          `${process.env.MEDUSA_BACKEND_API}/store/carts/${cart.id}/payment-session`,
          {
            provider_id: "stripe",
          }
        );
  ...
  async function createPaypalPaymentSession () {
    dispatch(setShowModal(true));
    dispatch(setLoading(true));
    document.body.style.overflow = "hidden";
    try {
      //select paypal payment session
      const res = await axios.post(
        `${process.env.MEDUSA_BACKEND_API}/store/carts/${cart.id}/payment-session`,
        {
          provider_id: "paypal",
    ...
    const getShippingOptions = async () => {
      try {
        const res = await axios.get(
          `${process.env.MEDUSA_BACKEND_API}/store/shipping-options/${cart?.id}`
        );
  ...
  const selectShippingOption = async (id: string) => {
    dispatch(setShowModal(true));
    dispatch(setLoading(true));
    document.body.style.overflow = "hidden";
    ...
    try {
      const res = await axios.post(
        `${process.env.MEDUSA_BACKEND_API}/store/carts/${cart.id}/shipping-methods`,
        {
          option_id: id,
        }
      );
      ...
      setShippingOption(
        res.data?.cart?.shipping_methods[0].shipping_option?.name
      );
      ...
      <div className="md:flex gap-6">
        <div className="flex-1">
          <div>
            ...
            <h3 className="text-Gray text-lg font-semibold my-3">
              Contact Information
            </h3>
          ...
          <h3 className="text-Gray text-lg font-semibold mt-3">
            Shipping Method
          </h3>
          ...
          {shippingOptions.map((option: any) => (
            <ShippingOption
              option={option}
              checked={shippingOption === option.name}
              onChange={(id: string) => {
                ...
                <div className="grid md:grid-cols-3 sm:grid-cols-2 gap-2">
                  {paymentMethods.map((method: any, i: number) => (
                    <PaymentMethod
                      ...
                      checked={
                        currentPaymentMethod == method.title ? true : false
                      }
